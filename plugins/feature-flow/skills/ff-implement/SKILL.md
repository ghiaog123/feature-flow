...
- **Opus/Sonnet (implementers)**: implementer sub-agents fan out to write code per the DAG — `ff-opus-implementer` (Opus, effort medium) for Medium/High-uncertainty phases, `ff-sonnet-implementer` (Sonnet) for Low/mechanical ones. Each agent owns its own files, receives a 5-part brief (objective/files with file:line anchors/interfaces/constraints/verification), self-verifies (capped loop) and returns a report with evidence.
...
- **Assign roles by cost**: Fable only does the "expensive reasoning" work (DAG analysis, verdicts on big decisions) — each call short and targeted. Implementers do all the code typing (~90% of the session's tokens). Don't use Fable to implement; don't use Opus to judge hard architectural decisions.
- **Pick the implementer lane by phase Uncertainty**: the plan labels each step's Uncertainty (Low/Medium/High). **Low/mechanical** (renames, config, boilerplate, regression runs) → `ff-sonnet-implementer` (Sonnet — much cheaper input, tighter loop). **Medium/High/architectural** → `ff-opus-implementer` (Opus, effort medium). Plan has no Uncertainty labels → default Opus. A Sonnet lane reporting `blocked` on an architectural ambiguity → rerun that phase on the Opus lane; the wrong-lane cost is one cheap failed attempt, not a wrong architecture.
- **Cost model to respect**: a sub-agent's cost ≈ context re-fed per turn × number of turns. Every file it reads is re-billed as input on every later turn — so briefs must scope reads narrowly (`file:line` anchors) and loops must be capped; that matters more than output size.
...
- **Fable analyzes the DAG**: spawn an agent with `model: 'fable'`, input = dependency contract + phase list + relevant plan excerpts. Task: build the DAG (node = phase, edge = `Depends on`), decide **which batches run in parallel, which sequentially**, flag phases touching shared files that must be merged/serialized, identify the **shared seam** to lock in Phase 0, and **assign each phase an implementer lane** from its Uncertainty label (Low/mechanical → `ff-sonnet-implementer`; Medium/High → `ff-opus-implementer`; no labels → all Opus). Concise output (DAG + lane per phase + reasoning), implements NOTHING.
...
2. **Fan out implementer sub-agents per Fable's DAG + lane assignment**: parallel batch → multiple sub-agents in 1 message (`subagent_type` per the lane Fable assigned: `'ff-sonnet-implementer'` for Low/mechanical phases, `'ff-opus-implementer'` for the rest); sequential batch → run along dependency edges. Each sub-agent receives a **self-sufficient 5-part brief** (the implementer doesn't see the conversation — the brief must stand on its own):
   1. **Objective** — phase goal + related Outcomes + that phase's plan excerpt.
   2. **Files** — `Owns files` (may only edit these files) **with mandatory `file:line` anchors** for every region the phase touches or must understand (pull them from the plan's affected-areas section; missing there → main greps them out before briefing). The anchors ARE the agent's read scope — the brief must state "read the anchored regions, do not read whole files >400 lines unless the anchored region can't explain itself".
   ...
   5. **Verification** — command proving the phase works; require running it once and putting real output in the report. **Cap: if it fails, at most 2 fix-and-rerun rounds, then report `partial` with the error** — no grinding. For a **large phase or an expensive/broad test suite**, split verification: the agent runs only the narrow phase-scoped check (or none), and **main runs the full suite itself at the integration join** — main paying for one test run is far cheaper than the agent re-feeding its whole accumulated context every retry loop.
...
3. **Fable advisor consult protocol** (applies to each Opus sub-agent — the Sonnet lane never consults: it has no Agent tool; on architectural ambiguity it reports `blocked` and main reruns the phase on the Opus lane):
...
For each issue Codex raises: valid → Claude **fixes the code**, verifies, then resumes; invalid → rebut with concrete evidence. Loop until `verdict === APPROVE` or stalemate, **hard cap 5 rounds** — past the cap, treat as stalemate: list the open points for the user instead of burning more resume rounds. Codex does NOT edit files. Always finalize+stop.
...
- ❌ Defaulting every phase to Opus when the plan carries Uncertainty labels — Low/mechanical phases belong on the Sonnet lane; input price × dozens of turns is where the money goes.
- ❌ Briefs whose Files section lists bare filenames without `file:line` anchors → the agent "reads the whole file to understand" and re-bills it every turn; anchors are mandatory.
- ❌ Letting a sub-agent grind a failing test suite beyond 2 fix rounds, or making an agent own a broad/expensive suite that main should run once at the integration join.
