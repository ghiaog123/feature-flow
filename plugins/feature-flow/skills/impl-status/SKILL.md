...
6. **Tiêu chí nghiệm thu** — acceptance criteria kiểm chứng được; `test-case-writer` + `feature-brief` map thẳng từ đây.
7. **Đánh giá** — Ưu / Rủi ro / Alternative đã loại.
8. **Checklist triển khai** high-level.

**Logic kĩ thuật → vẽ diagram ĐƠN GIẢN cho dễ nhìn, không trình bày bằng text dài.** Template nhúng sẵn Mermaid.js (CDN). Dùng block `<pre class="mermaid">` trong `<div class="diagram">`:
...
Quy tắc: nếu một đoạn mô tả >4–5 bước tuần tự / nhiều nhánh / nhiều quan hệ → chuyển sang diagram. Chọn loại diagram đơn giản nhất biểu diễn đủ logic.
