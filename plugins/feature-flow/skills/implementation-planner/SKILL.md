...
```
Quyết định/vấn đề (thường là analysis_brief.md từ problem-solver)
   │
   ▼
...
- **Bám giải pháp đã chốt (BẮT BUỘC)**: plan chỉ lo *cách hiện thực* quyết định đã chốt, KHÔNG trôi/đổi/thêm/bớt phạm vi. Nếu deep-dive phát hiện giải pháp đã chốt **không chạy được** → **DỪNG, báo ngược về `problem-solver`** để chốt lại; KHÔNG tự quyết lại giải pháp trong skill này. (Fidelity ngược lên upstream.)
...
- Nếu user vừa chạy `problem-solver` → **lấy `analysis_brief.md` làm đầu vào chính** (quyết định + lý do + đánh đổi + bước tiếp theo + ràng buộc). Đây là nguồn sự thật; plan phải bám nó.
- Nếu chưa có brief → hỏi user phát biểu: cần làm gì, tiêu chí thành công, ràng buộc (stack, không được đổi gì, deadline). Nếu vấn đề còn đang phân vân nhiều phương án → gợi ý chạy `problem-solver` trước, đừng lập plan trên nền chưa chốt.
...
**Kiểm fidelity ngay tại đây**: nếu deep-dive cho thấy giải pháp đã chốt trong brief **mâu thuẫn với code thật / không khả thi** → DỪNG, nêu rõ vì sao, báo ngược về `problem-solver`. Không âm thầm đổi giải pháp rồi lập plan cho cái khác.
...
**Dependency contract** (`Owns files` / `Depends on` cho mỗi bước) là hợp đồng để `plan-implementer` parallelize an toàn: bước có file disjoint + không phụ thuộc → chạy song song; còn lại tuần tự. Khai **honest** — nghi thì để tuần tự.
...
- **Chain**: `implementation_plan.md` APPROVE là đầu vào cho `plan-implementer` (hiện thực + review code). Nhắc bước kế tiếp của bundle: hiện thực → `impl-status` (tracking), `test-case-writer`/`service-test-runner` (test), `feature-brief` (bàn giao PO/QC).
...
- ❌ **Trôi khỏi giải pháp đã chốt** — tự đổi/thêm/bớt phạm vi thay vì báo ngược về `problem-solver`.
